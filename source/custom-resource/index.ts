...
import { CopyObjectCommand, CopyObjectCommandInput, GetObjectCommand, GetObjectCommandInput, PutObjectCommand, PutObjectCommandInput, S3Client, S3ClientConfig } from "@aws-sdk/client-s3";
import { DescribeEndpointCommand, DescribeEndpointCommandInput, DetachPolicyCommand, DetachPolicyCommandInput, IoTClient, IoTClientConfig, ListTargetsForPolicyCommand } from "@aws-sdk/client-iot";
import { UserAgent } from "@aws-sdk/types";

const { SOLUTION_ID, SOLUTION_VERSION } = process.env;
let s3ClientOptions: S3ClientConfig = {};
let iotClientOptions: IoTClientConfig = {};
if (SOLUTION_ID && SOLUTION_VERSION && SOLUTION_ID.trim() !== "" && SOLUTION_VERSION.trim() !== "") {
  const solutionUserAgent = `AwsSolution/${SOLUTION_ID}/${SOLUTION_VERSION}`;
  const s3Capability = `AwsSolution-Capability/${SOLUTION_ID}-C006/${SOLUTION_VERSION}`;
  const iotCapability = `AwsSolution-Capability/${SOLUTION_ID}-C007/${SOLUTION_VERSION}`;
  const s3CustomUserAgent : UserAgent = [[`${solutionUserAgent}`], [`${s3Capability}`]];
  const iotCustomUserAgent: UserAgent = [[`${solutionUserAgent}`], [`${iotCapability}`]];
  s3ClientOptions.customUserAgent = s3CustomUserAgent;
  iotClientOptions.customUserAgent = iotCustomUserAgent;
}
const s3Client = new S3Client(s3ClientOptions);
const iotClient = new IoTClient(iotClientOptions);
 ...
 */
exports.handler = async (event: CustomResourceTypes.EventRequest, context: CustomResourceTypes.LambdaContext):
Promise<CustomResourceTypes.CustomResourceResponse> => {
    ...
    switch (Resource) {
      ...
      case CustomResourceTypes.ResourceTypes.CREATE_UUID:
        if (RequestType === CustomResourceTypes.RequestTypes.CREATE) {
          ...
          response.Data.REDUCED_STACK_NAME = StackName.length < 10 ? StackName : StackName.substring(0, 9);
        }
        break;

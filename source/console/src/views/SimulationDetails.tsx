...
export default function SimulationDetails(props: IPageProps): JSX.Element {
    ...
    useEffect(() => {
        if (map && map.isStyleLoaded()) {
            let uniqueDeviceMessages = messages.filter((message, index, self) => self.findIndex((uniqueMsg) => uniqueMsg.content._id_ === message.content._id_) === index);
            let centerLat = 0;
            let centerLng = 0;
            let coordinates = uniqueDeviceMessages.map((message) => {
                centerLat = centerLat + message.content.location.latitude;
                centerLng = centerLng + message.content.location.longitude;
                return [message.content.location.longitude, message.content.location.latitude]
            });
    ...
    useEffect(() => {
        const iotSub = PubSub.subscribe(Object.keys(topics)).subscribe({
            next: (data: any) => { handleMessage(data); },
        ...
        shouldRedirect ? <Redirect to="/404" /> :
            <div className="page-content">
                ...
                <Card className="content-card">
                    <Card.Title className="content-card-title">{sim?.name}
                        <Button
                        ...
                        <Row>
                            <Col sm="6">
                                <Row className="detail">
                                    <Col sm="3"><b>{I18n.get("name")}:</b></Col>
                                    <Col sm="9">{sim?.name}</Col>
                                </Row>
                                <Row className="detail">
                                    <Col sm="3"><b>{I18n.get("stage")}:</b></Col>
                                    <Col sm="9">{sim?.stage}</Col>
                                </Row>
                                <Row className="detail mb-0">
                                    <Col sm="3"><b>{I18n.get("created")}:</b></Col>
                                    <Col sm="9">{sim?.createdAt}</Col>
                                </Row>
                            </Col>
                            <Col sm="6">
                                <Row className="detail">
                                    <Col sm="3"><b>{I18n.get("runs")}:</b></Col>
                                    <Col sm="9">{sim?.runs}</Col>
                                </Row>
                                <Row className="detail">
                                    <Col sm="3"><b>{I18n.get("last.run")}:</b></Col>
                                    <Col sm="9">{sim?.lastRun}</Col>
                                </Row>
                                <Row className="detail mb-0">
                                    <Col sm="3"><b>{I18n.get("last.updated")}:</b></Col>

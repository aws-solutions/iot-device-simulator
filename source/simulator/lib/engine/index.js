...
if (SOLUTION_ID && VERSION && SOLUTION_ID.trim() && VERSION.trim()) {
    awsOptions.customUserAgent = `AwsSolution/${SOLUTION_ID}/${VERSION}`;
}
...
class Engine {
    ...
    async start() {
        ...
        for (let i = 0; i < this.device.amount; i++) {
            ...
            if (this.device.states) {
                ...
                this.device.info.id =
                    `${this.simulation.simId.slice(0, 3)}${this.device.typeId.slice(0, 3)}${i}`;
            }
    ...
    async _pollDeviceStage() {
        ...
        try {
            let simulation = await docClient.get(params).promise();
            if (simulation.Item) {
